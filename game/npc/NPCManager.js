...
import { FRAME_THICKNESS, FRAME_TOP, COUNTER_HEIGHT } from '../ui/TianguisRenderer.js?v=6';
...
export class NPCManager {
    ...
    spawnNPC() {
        ...
        // Spawn from edges of the frame opening
        const { width, height } = this.scene.cameras.main;
        const viewL = FRAME_THICKNESS + 10;
        const viewR = width - FRAME_THICKNESS - 10;
        const viewT = FRAME_TOP + 40;
        const viewB = height - COUNTER_HEIGHT - 10;
        const side = Math.random();
        let startX, startY;
        if (side < 0.5) { startX = viewL - 20; startY = viewT + Math.random() * (viewB - viewT); }
        else { startX = viewR + 20; startY = viewT + Math.random() * (viewB - viewT); }

        // Destination slots within the visible frame area
        const destinations = [
            { x: viewL + 60,  y: viewT + 70 },
            { x: viewL + 200, y: viewT + 50 },
            { x: viewL + 350, y: viewT + 80 },
            { x: viewR - 80,  y: viewT + 60 },
            { x: viewL + 120, y: viewB - 50 },
            { x: viewL + 300, y: viewB - 30 },
            { x: viewR - 120,  y: viewB - 60 }
        ];
        ...
        // Hover effects — SUBTLE (no layout breaking)
        sprite.on('pointerover', () => {
            this.scene.tweens.add({
                targets: sprite, scaleX: npcEntry.baseScale * 1.05, scaleY: npcEntry.baseScale * 1.05, duration: 150, ease: 'Back.easeOut'
            });
    ...
    showNPCLabel(npcData, sprite) {
        ...
        const x = sprite.x;
        // Position label above sprite head, accounting for sprite scale
        const spriteH = sprite.height * sprite.scaleY;
        const y = sprite.y - spriteH - 15;
