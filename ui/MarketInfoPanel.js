...
export function showMarketInfo(scene, economySystem, marketEvents) {
    ...
    drawWarmOverlayPanel(bg, panelX, panelY, panelW, panelH, THEME.colors.lonaSecundaria);
    scene.marketPanel.push(bg);

    const title = scene.add.text(panelX + panelW / 2, panelY + 25, '📊 Estado del Mercado', {
        fontSize: '16px', fontFamily: THEME.fonts.main, fontStyle: 'bold', color: '#2E86AB'
    ...
    for (const [, cat] of Object.entries(overview)) {
        ...
        const factorColor = cat.factor > 1.2 ? '#4CAF50' : cat.factor < 0.8 ? '#C0392B' : '#F0C040';

        const label = scene.add.text(panelX + 20, y, `${cat.label}`, {
            fontSize: '13px', fontFamily: THEME.fonts.main, fontStyle: 'bold', color: '#E8DDD0'
        }).setDepth(9981);
        ...
        const factor = scene.add.text(
            ...
            { fontSize: '13px', fontFamily: THEME.fonts.main, color: factorColor }
        ).setOrigin(1, 0).setDepth(9981);
        scene.marketPanel.push(factor);

        const bar = scene.add.graphics().setDepth(9981);
        bar.fillStyle(THEME.colors.maderaOscura, 0.6);
    ...
    if (events.length > 0) {
        const evTitle = scene.add.text(panelX + panelW / 2, y + 5, '⚡ Eventos Activos', {
            fontSize: '13px', fontFamily: THEME.fonts.main, fontStyle: 'bold', color: '#F0C040'
        }).setOrigin(0.5).setDepth(9981);
        ...
        for (const ev of events) {
            ...
            const evText = scene.add.text(panelX + 20, y + 5, `${ev.icon} ${ev.name}`, {
                fontSize: '11px', fontFamily: THEME.fonts.main, color: '#F0C040'
            }).setDepth(9981);
    ...
    const closeBtn = scene.add.text(panelX + panelW - 25, panelY + 10, '✕', {
        fontSize: '18px', fontFamily: THEME.fonts.main, color: '#C0392B'
